...
class CategoriesController extends Controller
{
    ...
    protected function grid()
    {
        ...
        $grid->id('编号')->sortable();
        $grid->name('名称');
        $grid->is_directory('是否有子类目')->display(function ($value){
            return $value ? '是': '否';
        });
        $grid->level('层级');
        $grid->path('类目路径');
    ...
    protected function form($isEditing=false)
    {
        ...
        //如果是编辑的情况
        if($isEditing){
            //不允许用户修改parent_id ,is_directory这两个字段
            //用display方法来展示值,with方法接受一个匿名函数
            //会把字段值会传给匿名函数并返返回值展示出来
            ...
            //定义一个名为是否有子类目的单选框
            $form->radio('is_directory','是否有子类目')->options(['1'=>'是','0'=>'否'])
    ...
    public function apiIndex(Request $request)
    {
        ...
        //把查询出来的结果重新组装成laravel_admin需要的格式
